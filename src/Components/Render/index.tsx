    ...
    return (
        ...
        >
            <div className="min-h-screen flex flex-col">
                <div className="p-4 md:pt-6 pt-2 md:text-4xl text-2xl font-bold text-gray-800 mt-4">
                    Рестораны
                </div>
                ...
                {load ? (
                    ...
                    <div className="flex flex-row flex-wrap gap-5 p-4 lg:justify-center justify-between w-full mb-20">
                        {restaurants.map((item: Rest) => (
                            <div
                                key={uuidv4()}
                                className="flex flex-col rounded-xl w-full md:w-1/4 sm:w-1/3 lg:w-1/5 hover:scale-95 transition hover:duration-700"
                            >
                                <Link to={`/rest/${item.slug}`}>
                                    <div className="flex flex-col relative mb-2">
                                        <img
                                            alt=""
                                            src={item.image}
                                            className="w-full h-44 rounded-lg object-cover z-0"
                                        ></img>
                                    ...
                                    <div className="flex flex-col pb-4 gap-1 border-l-2 pl-2 h-fit pb-0 border-yellow-300">
                                        ...
                                        <p className="text-slate-700 border-2 border-yellow-300 rounded-2xl p-1 pl-2 pr-4 font-semibold w-fit">
                                            🥙 {item.cuisine}
                                        </p>
                                        <p className="text-slate-700 p-1 pl-2 font-semibold w-fit text-base">
                                            ☎️ {item.phone}
                                        </p>
                                        <p className="text-slate-700 bg-slate-100 rounded-xl p-1 pl-2 font-semibold w-fit lg:flex hidden flex-wrap text-xs">
                                            ✉️ {item.email}
                                        </p>
                                        <p className="text-sm font-semibold pr-4 text-slate-700">
                                            {' '}
                                            {item.address}
                                        </p>
                                    </div>
