     ...
     return(
        <div className="min-h-screen flex flex-col">
            ...
            <div className="w-full flex flex-row pt-6 mb-10">
                <Link to='/rest'>
                    <div className="w-fit pl-8 pt-1 flex items-center h-full text-2xl">
                        <ArrowLeftOutlined />
                    </div>
                </Link>
                ...
                <div className="flex flex-col pt-4 items-center w-full mb-20">  
                    {cartItems.map((item:any) => (
                        <div key={nanoid()} className="flex flex-col rounded-xl w-1/2 p-3 pr-10 pl-8 bg-yellow-100 m-auto mb-4">
                            <div className="flex flex-row justify-start pl-2 h-fit items-center">
                                ...
                                <p className="text-md font-semibold pr-4 text-slate-700 ml-4 w-1/6 text-center">{Math.round(item.price * item.quantity )} ₽</p>
                                <div onClick={() => removeFromCart(item.name)} className="ml-auto cursor-pointer">➖</div>
                                <p className="text-md font-normal pr-4 text-slate-700 rounded-full bg-white flex justify-center p-2 px-4 border-2 border-yellow-400 ml-1 mr-1"> {item.quantity } </p>
                    ...
                    {cartItems.length !== 0 &&
                        <div className="flex flex-row justify-center w-1/2 m-auto">
                            <p className="w-fit border-2 border-yellow-400 p-3 rounded-xl h-fit mr-auto"> Сумма заказа: {itemsquantity(cartItems)}  ₽</p>
